...
\begin{document}
	...
	\begin{definition}
		\normalfont
		Let $ \mathcal{V} $ be a set and $ \operation $ denote a binary operation between elements of this set such that $ \operation : \set{G} \times \set{G} \rightarrow \set{G} $. $G := (\set{G}, \operation)$ is called a group  \cite[page 36]{mml_book} if the following conditions are met and specific elements are present:
		\begin{enumerate}
