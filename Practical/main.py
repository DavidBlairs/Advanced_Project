...
class image_svd(object):
    ...
    def plot_singular_rgb(self, num_components: int) -> None:
        a, s_r, b = np.linalg.svd(test.get_component(0))
        a, s_g, b = np.linalg.svd(test.get_component(1))
        a, s_b, b = np.linalg.svd(test.get_component(2))

        position = np.arange(num_components)
        width = 0.2

        s_r = np.round(s_r ** 2 / np.sum(s_r ** 2), decimals=3)
        s_g = np.round(s_g ** 2 / np.sum(s_g ** 2), decimals=3)
        s_b = np.round(s_b ** 2 / np.sum(s_b ** 2), decimals=3)

        # Plot the bars
        plt.bar(position - width, s_r[:num_components], width=width, color='r', align='center', label='Red Component')
        plt.bar(position, s_g[:num_components], width=width, color='g', align='center', label='Green Component')
        plt.bar(position + width, s_b[:num_components], width=width, color='b', align='center', label='Blue Component')
